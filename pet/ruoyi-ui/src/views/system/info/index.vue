  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="宠物类型" prop="type">
        <el-select v-model="queryParams.type" placeholder="请选择宠物类型" clearable>
          <el-option
            v-for="dict in dict.type.sys_pet_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="性别" prop="gender">
        <el-select v-model="queryParams.gender" placeholder="请选择性别" clearable>
          <el-option
            v-for="dict in dict.type.sys_pet_gender"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
          <el-option
            v-for="dict in dict.type.sys_pet_status"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange">
      ...
      <!-- <el-table-column label="宠物ID" align="center" prop="petId" /> -->
             <!-- <el-table-column label="宠物图片URL" align="center" prop="imageUrl" /> -->
       <!-- 修改后：显示图片 -->
      ...
      <el-table-column label="宠物类型" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_pet_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
      <el-table-column label="性别" align="center" prop="gender">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_pet_gender" :value="scope.row.gender"/>
        </template>
      </el-table-column>
      <el-table-column label="年龄" align="center" prop="age" />
      <el-table-column label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_pet_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column label="宠物描述" align="center" prop="description" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="宠物图片" prop="imageUrl">
          <el-input v-model="form.imageUrl" placeholder="请添加宠物图片" />
        </el-form-item>
...
export default {
  name: "Info",
  dicts: ['sys_pet_status', 'sys_pet_gender', 'sys_pet_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
